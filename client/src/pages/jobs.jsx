...
const Hero = () => (
  <div className="relative bg-[#1aac83] px-8 pt-10 pb-24 flex justify-between items-center overflow-visible rounded-t-xl">
    <div className="max-w-2xl">
      <h1 className="text-5xl font-serif text-white mb-6">Find Your Dream Job</h1>
    </div>
  ...
  <div className="max-w-6xl mx-auto -mt-8 px-4 relative z-20">
    <div className="bg-white rounded-lg shadow-lg p-3 flex flex-col md:flex-row items-center gap-4">
      <div className="flex-1 flex items-center justify-center gap-3 px-4 border-b md:border-b-0 md:border-r border-gray-200 py-2 w-auto">
        <Search className="text-gray-400 w-5 h-5" style={{marginTop:"10px",marginBottom:"20px"}} />
        <input 
          type="text" 
      ...
      <div className="flex-1 flex items-center gap-3 px-4 py-2 w-full">
        <MapPin className="text-gray-400 w-5 h-5" style={{marginTop:"10px",marginBottom:"20px"}} />
        <input 
          type="text" 
      ...
      <button style={{marginTop:"10px",marginBottom:"20px"}} className="bg-[#286ed6] hover:bg-[#286ed6] text-white font-semibold px-10 py-3 rounded transition shadow-md w-full md:w-auto">
        Search
      </button>
    </div>
  </div>
...
const Checkbox = ({ label, checked, onChange }) => (
  ...
  <label className="inline-flex items-start gap-3 cursor-pointer group">
    <div className={`w-5 h-5 rounded border flex items-center justify-center transition ${checked ? 'bg-[#286ed6] border-[#a6bcdb]' : 'border-gray-300 bg-white'}`}>
      {checked && <div className="w-2 h-2 bg-white rounded-sm" />}
    </div>
    <span className="text-gray-600 text-sm group-hover:text-gray-800 transition">{label}</span>
      ...
      <main className="max-w-6xl mx-auto px-4 py-12 grid grid-cols-1 md:grid-cols-12 gap-">
        ...
        <div className="md:col-span-3">
          ...
          <FilterSection title="Experience Level">
            <Checkbox label="Entry/Junior" checked={filters.junior} onChange={() => {}} />
            <Checkbox label="Middle" checked={filters.middle} onChange={() => {}} />
            <Checkbox label="Senior" checked={filters.senior} onChange={() => {}} />
          </FilterSection>
        </div>


        <div className="md:col-span-9 overflow-auto px-3" style={{height:"100vh"}}>
          <div className="flex justify-between items-center mb-6">
            <p className="text-gray-400 text-sm">Showing 68 results</p>
