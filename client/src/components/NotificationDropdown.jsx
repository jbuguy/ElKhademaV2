...
const NotificationDropdown = () => {
  ...
  const hasMoreNotifications = !showAll && notifications.filter(n => n.read).length > 0;
  ...
  const getNotificationContent = (notification) => {
    ...
    switch (notification.type) {
      ...
      case 'comment':
        ...
        break;
      case 'share':
        icon = <FaShare className="text-green-500" />;
  ...
  return (
    <div ref={dropdownRef} className="relative">
      <button
        ...
        className="relative border-0 bg-transparent p-0 text-gray-600 hover:text-[#1aac83] transition-colors duration-200"
      >
        <FaBell size={20} />
        {unreadCount > 0 && (
      ...
      {isOpen && (
        <div className="absolute right-0 top-12 w-[380px] bg-white rounded-xl shadow-2xl border border-gray-200 z-50 max-h-[70vh] flex flex-col">
          <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-[#1aac83] to-[#15937a] shrink-0">
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 border-b border-gray-100 hover:bg-gray-50 cursor-pointer transition-all duration-200 flex items-start ${
                        !notification.read ? 'bg-blue-50 border-l-4 border-l-[#1aac83]' : ''
                      }`}
                    >
                      <div className="flex items-start gap-3 w-full">
                        <div className={`shrink-0 w-12 h-12 ${color} rounded-full flex items-center justify-center text-lg shadow-sm`}>
                          {icon}
                        </div>
                        <div className="w-[280px] flex flex-col gap-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {!notification.read && (
                              <span className="w-2 h-2 bg-[#1aac83] rounded-full"></span>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            <div className="flex gap-2 p-3 border-t border-gray-200 bg-gray-50 shrink-0">
              {!showAll && hasMoreNotifications && (
                <button
                  ...
                  className="flex-1 py-2.5 px-4 text-sm font-semibold text-[#1aac83] hover:bg-white border-2 border-[#1aac83] rounded-lg transition-all duration-200 whitespace-nowrap"
                >
                  See More
                </button>
              )}
              ...
              {unreadCount > 0 && (
                <button
                  ...
                  className="flex-1 py-2.5 px-4 text-sm font-semibold text-white bg-[#1aac83] hover:bg-[#158f6b] rounded-lg transition-all duration-200 shadow-sm whitespace-nowrap"
                >
                  Mark All Read
                </button>
              )}
