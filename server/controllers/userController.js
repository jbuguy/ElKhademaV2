...
export const loginUser = async (req, res) => {
  try {
    ...
    res.status(200).json({ token, email: user.email, profilePic: user.profilePic, _id: user._id });
  } catch (error) {
...
export const registerUser = async (req, res) => {
  try {
    ...
    const token = generateToken(user);
    res.status(200).json({ token, email: user.email, profilePic: user.Profile.Pic, _id: user._id });
  } catch (error) {
    res.status(500).json({ error: error.message });
    console.log(error);
  }
